...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const prodRes = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/seller/products`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setProducts(prodRes.data);

        const orderRes = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/seller/orders`, {
          headers: { Authorization: `Bearer ${token}` }
        });
